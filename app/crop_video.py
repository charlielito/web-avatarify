...
with PythonPath("."):
    from app.io import get_frames_from_camera
...
def compute_bbox(start, end, fps, tube_bbox, frame_shape, inp, increase_area=0.1):
    ...
    top, bot, left, right = (
        max(0, top),
        min(bot, frame_shape[0]),
        max(0, left),
        min(right, frame_shape[1]),
    )
...
def process_video(args):
    ...
    fps = 30
    video = get_frames_from_camera(0)
    ...
    try:
        for i, frame in tqdm(enumerate(video)):
            ...
            print(commands, bboxes, trajectories)
            ...
            for bbox in bboxes:
                ...
                for trajectory in trajectories:
                    ...
                    if (
                        ...
                        and current_intersection > args.iou_with_initial
                    ):
                        intersection = bb_intersection_over_union(tube_bbox, bbox)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--increase", default=0.1, type=float, help="Increase bbox by this amount"
    )
    parser.add_argument(
        "--iou_with_initial",
        type=float,
    ...
    parser.add_argument(
        "--min_frames", type=int, default=150, help="Minimum number of frames"
    )
